 ...
 * @type {symbol}
 */
const collectionSymbol = Symbol.for('c_collection');

/**
 * @private
 * @type {symbol}
 */
const pathSymbol = Symbol.for('c_path');

/**
 * @private
 * @type {symbol}
 */
const modelSymbol = Symbol.for('c_model');
...
class Model extends EventTarget {
   ...
   */
  constructor(attributes, { collection } = opt) {
    super();
  ...
  merge(source, target = this) {
    Object.keys(source).forEach((key) => {
      ...
      target[key] =
        isObject(existing) && isObject(current)
  ...
  write(options = opt) {
    return this.sync('write', options)
      .then((response) => {
        if (isObject(response) && !options.skip) {
          const method = options.method in this ? options.method : 'assign';
          this[method](response);
        }
        this.dispatchEvent(
   ...
   */
  sync(method, options) {
    const collection = this[collectionSymbol];
    const storage =
      collection && collection.storage ? collection.storage : this.constructor.storage;
  ...
  static getProxy(target, path, model, processed) {
    ...
    Object.keys(target).forEach((key) => {
      if (isObject(target[key]) && !processed.includes(target[key])) {
...
Model.proxyHandler = {
  ...
  set(target, property, value, receiver) {
    ...
    target[property] = isObject(value)
