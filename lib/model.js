...
class Model extends Listener() {
  ...
  merge(source, target = this) {
    Object.keys(source).forEach((key) => {
      ...
      target[key] =
        _isObject(existing) && _isObject(current)
  ...
  read(options = _opt) {
    return this.sync('read', options)
      .then((response) => {
        ...
        if (!options.silent)
          this.dispatchEvent(
            new CustomEvent('sync', { detail: { emitter: this, response, options } }),
          );
        return response;
      })
      .catch((error) => {
  ...
  sync(method, options) {
    ...
    const storage =
      collection && collection.storage ? collection.storage : this[Symbol.for('c_storage')];
...
Model.proxyHandler = {
  ...
  set(target, property, value, receiver) {
    ...
    target[property] = _isObject(value)
      ? Model._getProxy(value, `${path}:${property}`, model, [value])
      : value;
