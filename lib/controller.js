...
const cFragment = Symbol.for('c_fragment');
const cModelAttributes = Symbol.for('c_modelAttributes');
const cHandlers = Symbol.for('c_handlers');
...
class Controller extends HTMLElement {
  ...
  constructor({ model } = _opt) {
    ...
    this[cFragment] = '';
    this[cModelAttributes] = observedAttributes.filter((attribute) => attribute.includes(':'));

    if (this[cModelAttributes].length) {
      this._onModelChange = this._onModelChange.bind(this);
    }

    this[cHandlers] = handlers ? this._prepareHandlers(handlers) : undefined;
  ...
  addEventListener(name, callback, options = _opt) {
    ...
    if (!this[cHandlers]) this[cHandlers] = new Map();
    let event = this[cHandlers].get(name);
    let cb = callback;
    if (typeof cb === 'string') cb = this[cb];
    if (typeof cb !== 'function') return;
    if (!event) {
      event = this[cHandlers].set(name, []).get(name);
  ...
  removeEventListener(name, callback, options = _opt) {
    ...
    const handlers = this[cHandlers] && this[cHandlers].get(name);
    if (!handlers) return;
    if (!selector) {
   ...
   */
  onNavigate(event) {
    const path = event && event.target && event.target.href;
    if (!path) return;
  ...
  dispose({ silent, save } = _opt) {
    ...
    if (this.model && this[cModelAttributes].length) {
  ...
  _onModelChange({ detail: { path, previous } }) {
    if (!this[cModelAttributes].some((name) => path.startsWith(name))) return;
