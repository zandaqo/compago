...
class ModelArray extends Listener(Array) {
  ...
  prepareModel(data) {
    ...
    return new this.Model(data, { collection: this });
  }
   ...
   */
  addReference(model) {
    if (!model[collectionSymbol]) model[collectionSymbol] = this;
  ...
  removeReference(model) {
    ...
    if (model[collectionSymbol] === this) model[collectionSymbol] = undefined;
